...
{
    void fe_create_hamming(float64*,int32);
	...
	if ((FE->FB = (melfb_t *) calloc(1,sizeof(melfb_t)))==NULL){
	    fprintf(stderr,"memory alloc failed in fe_init()\n...exiting\n");
	    return(NULL);
	}
	/* transfer params to mel fb */
	fe_parse_melfb_params(P, FE->FB);

	fe_build_melfilters(FE->FB);
	fe_compute_cosine(FE->FB);
    } 
    else if (FE->FB_TYPE==LINEAR) {   
	if ((FE->FB = (melfb_t *) calloc(1,sizeof(melfb_t)))==NULL){
	    fprintf(stderr,"memory alloc failed in fe_init()\n...exiting\n");
	    return(NULL);
	}
	fe_parse_melfb_params(P, FE->FB);
	fe_build_linearfilters(FE->FB);
	fe_compute_cosine(FE->FB);
    }
    else {
	fprintf(stderr,"MEL SCALE IS CURRENTLY THE ONLY IMPLEMENTATION (1)!\n");
	return(NULL);
    }
...
int32 fe_process_utt(fe_t *FE, int16 *spch, int32 nsamps, float ***cep_block)
{
    ...
    double *spbuf=NULL, *fr_data, *fr_fea;
    int16 *tmp_spch = spch;
    float **cep=NULL;
	...
	frame_count++;
      ...
      if (FE->CEPFEAT)
          cep = (float **)fe_create_2d(frame_count,FE->NUM_CEPSTRA,sizeof(float));
      else
          cep = (float **)fe_create_2d(frame_count,FE->FB->num_filters,sizeof(float));
	...
	fe_short_to_double(tmp_spch, spbuf, spbuf_len);
      ...
      if (FE->CEPFEAT)
          ...
          fr_fea = (double *)calloc(FE->FB->num_filters, sizeof(double));
	...
	assert(FE->NUM_OVERFLOW_SAMPS<FE->FRAME_SIZE);
      ...
      if (spch != tmp_spch) {free (tmp_spch); tmp_spch = NULL;}
      
      free(spbuf);   spbuf = NULL;
      free(fr_data); fr_data = NULL;
      free(fr_fea);  fr_fea = NULL;
    }
...
int32 fe_close(fe_t *FE)
{
  ...
  if (FE->FB_TYPE==MEL_SCALE || FE->FB_TYPE==LINEAR) {
    fe_free_2d((void *)FE->FB->filter_coeffs);
    fe_free_2d((void *)FE->FB->mel_cosine);
    free(FE->FB->left_apex);
    free(FE->FB->width);
    free(FE->FB);
  } else {
    fprintf(stderr,"MEL SCALE AND LINEAR ARE CURRENTLY THE ONLY IMPLEMENTATIONS (2)!\n");
  }
