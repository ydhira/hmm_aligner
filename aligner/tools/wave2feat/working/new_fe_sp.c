...
int32 fe_build_linearfilters(melfb_t *FB)
{    
    int32 i, whichfilt; 

    /*estimate filter coefficients*/
    FB->num_filters = FB->fft_size/2 + 1;
    FB->filter_coeffs = (float32 **)fe_create_2d(FB->num_filters, 1, sizeof(float32));
    FB->left_apex = (int32 *) calloc(FB->num_filters,sizeof(int32));
    FB->width = (int32 *) calloc(FB->num_filters,sizeof(int32));
    
    if (FB->filter_coeffs==NULL || FB->left_apex==NULL || FB->width==NULL ){
	fprintf(stderr,"memory alloc failed in fe_build_linearfilters()\n...exiting\n");
	exit(0);
    }

    for (whichfilt=0;whichfilt<FB->num_filters; ++whichfilt)
    {
	FB->left_apex[whichfilt] = whichfilt;
	FB->width[whichfilt] = 1;
	FB->filter_coeffs[whichfilt][0] = 1;
    }
...
int32 fe_build_melfilters(melfb_t *FB)
{    
    ...
    /*estimate filter coefficients*/
    FB->filter_coeffs = (float32 **)fe_create_2d(FB->num_filters, FB->fft_size, sizeof(float32));
    FB->left_apex = (int32 *) calloc(FB->num_filters,sizeof(int32));
    FB->width = (int32 *) calloc(FB->num_filters,sizeof(int32));
    
    if (FB->doublewide==ON)
	filt_edge = (float32 *) calloc(FB->num_filters+4,sizeof(float32));
    else	
	filt_edge = (float32 *) calloc(FB->num_filters+2,sizeof(float32));

    
    if (FB->filter_coeffs==NULL || FB->left_apex==NULL || FB->width==NULL || filt_edge==NULL){
	fprintf(stderr,"memory alloc failed in fe_build_mel_filters()\n...exiting\n");
	exit(0);
    }
    
    dfreq = FB->sampling_rate/(float32)FB->fft_size;
    
    melmax = fe_mel(FB->upper_filt_freq);
    melmin = fe_mel(FB->lower_filt_freq);
    dmelbw = (melmax-melmin)/(FB->num_filters+1);
    
    if (FB->doublewide==ON){
	melmin = melmin-dmelbw;
	melmax = melmax+dmelbw;
	if ((fe_melinv(melmin)<0) ||
	    (fe_melinv(melmax)>FB->sampling_rate/2)){
    ...
    if (FB->doublewide==ON){
	for (i=0;i<=FB->num_filters+3; ++i){
	    filt_edge[i] = fe_melinv(i*dmelbw + melmin);   
	}

    }
    else{    
	for (i=0;i<=FB->num_filters+1; ++i){
	    filt_edge[i] = fe_melinv(i*dmelbw + melmin);   
	}
    }
	...
	FB->left_apex[whichfilt] = (int32)(leftfr/dfreq) + 1;
	...
	while (freq<=centerfr){
	    FB->filter_coeffs[whichfilt][i] = (freq-leftfr)*leftslope;	    
	    freq += dfreq;
	    i++;
	}
	while (freq<rightfr){
	    FB->filter_coeffs[whichfilt][i] = (freq-rightfr)*rightslope;
	    freq += dfreq;
	    i++;
	}
	
	FB->width[whichfilt] = i;
    }
...
int32 fe_compute_cosine(melfb_t *FB)
{

    float32 period, freq;
    int32 i,j;
    
    period = (float32)2*FB->num_filters;

    if ((FB->mel_cosine = (float32 **) fe_create_2d(FB->num_cepstra,FB->num_filters,
					      sizeof(float32)))==NULL){
	fprintf(stderr,"memory alloc failed in fe_compute_cosine()\n...exiting\n");
	exit(0);
    }
    
    
    for (i=0; i<FB->num_cepstra; i++) {
	freq = 2*(float32)M_PI*(float32)i/period;
	for (j=0;j< FB->num_filters;j++)
	    FB->mel_cosine[i][j] = (float32)cos((float64)(freq*(j+0.5)));	
    }    
...
void fe_frame_to_fea(fe_t *FE, float64 *in, float64 *fea)
{
    ...
    if (FE->FB_TYPE == MEL_SCALE || FE->FB_TYPE == LINEAR){
	spec = (float64 *)calloc(FE->FFT_SIZE, sizeof(float64));
	mfspec = (float64 *)calloc(FE->FB->num_filters, sizeof(float64));
	...
	fe_warp_spec(FE, spec, mfspec);
	fe_warp_cep(FE, mfspec, fea);
	
	free(spec);
	free(mfspec);	
    }
    else {
	fprintf(stderr,"MEL SCALE AND LINEAR ARE CURRENTLY THE ONLY IMPLEMENTATIONS (3)!\n");
	exit(0);
    }
...
void fe_warp_spec(fe_t *FE, float64 *spec, float64 *mfspec)
{
    int32 whichfilt, start, i;
    
    for (whichfilt = 0; whichfilt<FE->FB->num_filters; whichfilt++){
	start = FE->FB->left_apex[whichfilt];
	mfspec[whichfilt] = 0;
	for (i=0; i< FE->FB->width[whichfilt]; i++)
	    mfspec[whichfilt] +=
		FE->FB->filter_coeffs[whichfilt][i]*spec[start+i];	
    }
...
void fe_warp_cep(fe_t *FE, float64 *mfspec, float64 *mfcep)
{
    int32 i,j;
    static first_run=1;
    int32 period;
    ...
    period = FE->FB->num_filters;



    for (i=0;i<FE->FB->num_filters; ++i)
    {
	if (mfspec[i]>0)
    ...
    if (FE->CEPFEAT){
	    ...
	    mfcep[i] = 0;
	    for (j=0;j<FE->FB->num_filters; j++){
	        if (j==0)
		    beta = 0.5;
	        else
		    beta = 1.0;
	        mfcep[i] += beta*mfspec[j]*FE->FB->mel_cosine[i][j];
	    }
